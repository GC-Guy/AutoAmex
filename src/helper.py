...
old_amexWebsite = "https://online.americanexpress.com/myca/logon/us/action/LogonHandler?request_type=LogonHandler&Face=en_US&inav=iNavLnkLog"
...
def closeFeedback(driver):
	# Close AJAX popup for Amex feedback
	try: driver.find_element_by_class_name("srCloseBtn").click() 
	except: pass
	try: driver.find_element_by_class_name("fsrCloseBtn").click()
	except: pass
	try: driver.find_element_by_class_name('btnCancelLite').click()
	except: pass
	try: driver.find_element_by_class_name('uz_span_close').click()
	except: pass
	time.sleep(1)
